...
class MessageThread:
    ...
    def create_new_thread(self):
        """Creates new thread if the thread does not exits. Note: This endpoint does not work I added in because sony might fix it in future
        ...
        response = self.request_builder.multipart_post(
            url="{}/threads".format(MessageThread.base_uri),
            name="threadDetail",
            data=data,
        )
    ...
    def threads_with_online_ids(self, online_ids):
        ...
        response = self.request_builder.get(
            url="{}/users/me/threadIds".format(MessageThread.base_uri),
    ...
    def get_thread_information(self, count=1, thread_id=None):
        ...
        params = {
            "fields": "threadMembers,threadNameDetail,threadThumbnailDetail,threadProperty,"
            "latestTakedownEventDetail,newArrivalEventDetail,threadEvents",
            "count": count,
        }
        response = self.request_builder.get(
            url="{}/threads/{}".format(MessageThread.base_uri, thread_id), params=params
        )
        return response
    ...
    def leave_thread(self):
        ...
        self.request_builder.delete(
            url="{}/threads/{}/users/me".format(MessageThread.base_uri, self.thread_id)
        )
    ...
    def send_message(self, message):
        ...
        self.request_builder.multipart_post(
            url="{}/threads/{}/messages".format(MessageThread.base_uri, self.thread_id),
